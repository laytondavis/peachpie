...
namespace Peachpie.Library.Postgres
{
    ...
    /// </summary>
    [PhpHidden]
    sealed class PostgresqlConnectionResource : ConnectionResource
    {
        readonly PostgresConnectionManager _manager;
        readonly NpgsqlConnection _connection;

        public PostgresqlConnectionResource(PostgresConnectionManager manager, string connectionString) : base(manager.Context, connectionString, "Postgres connection")
        {
            _manager = manager;
            try
            {
            ...
            {
                PhpException.Throw(new PhpError(), e.ToString());
            }
        }

        protected override void FreeManaged()
        {
            // this is called from the postgres close function.
            // clean up the postgres connection
        ...
        {
            var myreader = (NpgsqlDataReader)_pendingReader;
            if (myreader != null)
            {
        ...
        {
            return new PostgresResultResource(connection, reader, convertTypes);
        }
