...
{
    internal sealed class PostgresResultResource : ResultResource
    {
        ///// <summary>
        ...
        public PostgresResultResource(ConnectionResource/*!*/ connection, IDataReader/*!*/ reader, bool convertTypes)
            : base(connection, reader, "Postgres result", convertTypes)
        {
            // no code in here
        }

        internal static PostgresResultResource ValidResult(PhpResource handle)
        {
            var result = handle as PostgresResultResource;
        ...
        {
            var reader = (NpgsqlDataReader)this.Reader;
            if (reader != null)
            {
        ...
        {
            var my_reader = (NpgsqlDataReader)Reader;
            ...
            if (convertTypes)
            {
                ...
                {
                    object mval = null;
                    if (dataTypes[i] == "date" && !my_reader.IsDBNull(i))
                    {
                        mval = my_reader.GetDate(i);
                    }
                    else if (dataTypes[i] == "timestamp" && !my_reader.IsDBNull(i))
                    {
                        mval = my_reader.GetTimeStamp(i);
                    }
                    else
                    {
                        mval = my_reader.GetValue(i);
                    }
                    oa[i] = ConvertDbValue(dataTypes[i], mval);
                }
            }
            else
            {
        ...
        private static object ConvertDbValue(string dataType, object sqlValue)
        {
            ...
            if (sqlValue.GetType() == typeof(NpgsqlDate))
                return ((NpgsqlDate)sqlValue).ToString();

            if (sqlValue.GetType() == typeof(NpgsqlDateTime))
                return ((NpgsqlDateTime)sqlValue).ToString();
